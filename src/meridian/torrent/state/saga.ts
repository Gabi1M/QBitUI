...
function* pauseTorrentsSaga(action: PauseTorrentsAction) {
    ...
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Torrents paused successfully!`,
                'success',
                2000
            )
        );
...
function* resumeTorrentsSaga(action: ResumeTorrentsAction) {
    ...
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Torrents resumed successfully!`,
                'success',
                2000
            )
        );
...
function* deleteTorrentsSaga(action: DeleteTorrentsAction) {
    ...
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Torrents deleted successfully!`,
                'success',
                2000
            )
        );
...
function* forceDownloadTorrentsSaga(action: ForceDownloadTorrentsAction) {
    ...
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Torrents successfully set to force download!`,
                'success',
                2000
            )
        );
        ...
        yield put(
            showSnackbarAction(
                t`Failed to set torrents to force download!`,
                'error',
                2000
            )
        );
...
function* recheckTorrentsSaga(action: RecheckTorrentsAction) {
    ...
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Failed to submit torrents recheck!`,
                'error',
                2000
            )
        );
