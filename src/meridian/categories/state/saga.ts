...
function* setCategoriesSaga(action: ResourceSetAction<Resource.CATEGORIES>) {
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Categories added successfully!`,
                'success',
                2000
            )
        );
        ...
        yield put(
            showSnackbarAction(t`Failed to add categories!`, 'success', 2000)
        );
    }
}

function* deleteCategoriesSaga(
    action: ResourceDeleteAction<Resource.CATEGORIES>
) {
    try {
        ...
        yield put(
            showSnackbarAction(
                t`Categories deleted successfully!`,
                'success',
                2000
            )
        );
        ...
        yield put(
            showSnackbarAction(t`Failed to delete categories!`, 'error', 2000)
        );
    }
}
