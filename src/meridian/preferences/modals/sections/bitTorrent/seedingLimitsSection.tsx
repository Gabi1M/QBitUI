...
const SeedingLimitsSection = ({ preferences, updatePreferencesKey }: SectionProps) => (
    <Accordion.Panel>
        <Switch
            ...
            onChange={(value) => updatePreferencesKey('max_ratio_enabled', value.target.checked)}
        />
        <NumberInput
            mt='md'
            disabled={!preferences?.max_ratio_enabled}
            value={preferences?.max_ratio || 0}
            onChange={(value) => updatePreferencesKey('max_ratio', value as number)}
        />
        <Switch
            mt='md'
            ...
            onChange={(value) =>
                updatePreferencesKey('max_seeding_time_enabled', value.target.checked)
            }
        />
        <NumberInput
            mt='md'
            ...
            onChange={(value) => updatePreferencesKey('max_seeding_time', value as number)}
        />
        <Select
            mt='md'
            label={t`Then`}
            disabled={!preferences?.max_ratio_enabled && !preferences?.max_seeding_time_enabled}
            value={BitTorrentMaxRatioActNameMaping[preferences?.max_ratio_act as number]}
