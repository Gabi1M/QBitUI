...
const ConnectionLimitsSection = ({ preferences, updatePreferencesKey }: SectionProps) => (
    <>
        <NumberInput
            ...
            onChange={(value) => updatePreferencesKey('max_connec', value as number)}
        />
        <NumberInput
            mt='md'
            label={t`Maximum number of connections per torrent`}
            value={preferences?.max_connec_per_torrent || 0}
            onChange={(value) => updatePreferencesKey('max_connec_per_torrent', value as number)}
        />
        <NumberInput
            mt='md'
            label={t`Global maximum number of upload slots`}
            value={preferences?.max_uploads || 0}
            onChange={(value) => updatePreferencesKey('max_uploads', value as number)}
        />
        <NumberInput
            mt='md'
